...
module.exports = {
  ...
  destroyDatabaseVolume: (app) => {
    return new Promise(async (resolve, reject) => {
      console.log('Deleting database volume...');
      const managerNode = await getManagerNodeInstance();
      
      const volume = managerNode.getVolume(`${app.title}_db_data`);

      volume.remove((error, result) => {
        if (error) {
          console.log(error);
          reject(error);
        }

        console.log('Database volume deleted!')
        resolve(app);
      });
    });
  },
