...
const cnbpDockerfile = (appName, filename) => {
  return `FROM dricevans/app-packer
COPY ${filename} /app/
WORKDIR /app
RUN unzip ${filename}
RUN rm ${filename}

CMD ["pack", "build", "${appName}-built", "--builder", "heroku/buildpacks"]`;
};

const appDockerfile = (appName) => {
  return `FROM ${appName}:latest
WORKDIR /app
ENV PORT=4567
EXPOSE 4567`;
};
...
module.exports = {
  ...
  buildDockerfile: (filename) => {
    return (app) => {
      return new Promise((resolve, reject) => {
        ...
        fs.writeFile(`./${app.path}/Dockerfile`, cnbpDockerfile(app.title, filename), (err) => {
          if (err) { reject(err); }
  ...
  buildSecondDockerfile: (app) => {
      return new Promise((resolve, reject) => {
        app.emitEvent('Building second Dockerfile...');
        fs.writeFile(
          `./${app.path}/Dockerfile.second`,
          appDockerfile(app.title),
          (err) => {
            if (err) { reject(err); }
            app.emitEvent('Built second Dockerfile!');
            resolve(app);
        });
      });
  },

  // Here, we build an image that, when run, creates an image
  // for the app. This image is based on an image that contains
  // CNBP pack and docker, it copies in the app source code,
  // and specifies a pack command as its default command
  buildPackerImage: (app) => {
    return new Promise(async (resolve, reject) => {
  ...
  // This function runs the packer image created in the previous
  // step to actually build an image for the app.
  buildTempAppImage: (app) => {
    const containerOpts = { 
      AttachStdout: false,
      // We need to mount the host's docker daemon into this container so
      // that pack can run docker commands
    ...
    return new Promise(async (resolve, reject) => {
      const managerNode = await getManagerNodeInstance();
      managerNode.run(
        `${app.title}-builder`, null, null, containerOpts,
        (err, data, container) => {
          if (err) {
            reject(err);
            return;
  ...
  // The image we built for the app from buildTempAppImage does not
  // expose any ports by default, so we build a final app image by
  // basing off that image, but exposing appropriate ports and setting
  // the PORT env variable so that the app launches listening on the
  // appropriate port, and that port is exposed to Docker
  buildFinalAppImage: (app) => {
    const tarStream = tar.pack(app.path);
    ...
    return new Promise(async (resolve, reject) => {
      ...
      docker.buildImage(
        tarStream,
        { t: app.title, dockerfile: 'Dockerfile.second' },
