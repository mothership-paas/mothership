...
module.exports = {
  ...
  buildDockerignoreFile: () => {
  console.log('Inside dockerignore file creator');
  	return (app) => {
  	  return new Promise((resolve, reject) => {
  	    console.log('app: ', app);
  	    
  	  	console.log('Building .dockerignore file...');
  	  	console.log(app.path)
  	  	console.log(dockerignoreFileContent());
  	  	fs.writeFile(`./${app.path}/.dockerignore`, dockerignoreFileContent(), (err) => {
  	  	  if (err) { throw(err); }
  	  	  console.log('Built .dockerignore file!');
  	  	  resolve(app);
  	  	});
  	  });
  	};
