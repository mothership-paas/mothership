...
wsAppLogs.on('connection', async function connection(ws, request) {
  ...
  service.logs(options, function handler(err, stream) {
    // I noticed when simply piping the stream to the WS client that weird artifacts
    // would sometimes present on lines.
    // See: https://github.com/moby/moby/issues/32794
    //
    // It turns out that Dockerode (or really, Docker Modem used internally) comes
    // with a nice piece of functionality for handling this called `demuxStream`
